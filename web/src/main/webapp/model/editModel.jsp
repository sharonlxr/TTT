					...
					<tr>
						...
						<td class="durabledetail">
							<s:if test="%{#item.valueList}">
								<s:select id="%{#item.name}" name="modelVO.modelAttributesValues['%{#item.name}']" theme="simple" list="modelVO.dropdownValues[#item.name]" style="width:260px"/>
							</s:if>
							<s:else>
								<s:if test="#item.required">
									<s:textfield required="required" theme="simple" name="modelVO.modelAttributesValues['%{#item.name}']" id="%{#item.name}" style="width:260px"/>
								</s:if>
								<s:else>
									<s:textfield theme="simple" name="modelVO.modelAttributesValues['%{#item.name}']" id="%{#item.name}" style="width:260px"/>
								</s:else>
							</s:else>
						</td>
						<s:if test="#st.last">
			...
			<table class="durabledetail" id="childDurable">
				<s:iterator value="modelVO.durableList" id="item" status="st">
					<s:if test="#st.odd">
						<tr>
							<td class="durabledetail">
								<input type="checkbox" id="selectChildrenList" name="modelVO.selectChildrenList" value="<s:property value='#item.mESName' />" />
